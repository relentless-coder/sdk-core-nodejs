...
function OAuth(consumerKey, p12Path, alias, password) {
    if(!utils.isSet(consumerKey)) {
        throw new error.APIError('Consumer key is not set');
    }

    if(!utils.isSet(p12Path)) {
        throw new error.APIError('Path to .p12 file is not set');
    }

    if(!utils.isSet(alias)) {
        throw new error.APIError('Key alias is not set');
    }

    if(!utils.isSet(password)) {
        throw new error.APIError('Keystore password is not set');
    }
...
var _getPrivateKey = function (p12Path, alias, password) {
    ...
    if(!utils.isSet(keyObj)) {
        throw new error.APIError("No key found for alias [" + alias + "]")
    }
